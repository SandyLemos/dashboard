...
const favoriteEventsData = [
  {
    ...
    description: 'Com texto, interpretação e direção de Raphael Ghanem, "Se é que você me entende" é um espetáculo de Stand up Comedy, repleto de interações e improvisações, feito com texto 100% autoral.',
    categoryLabel: "Stand up comedy",
    categoryColor: "bg-[#FFF3E0] text-[#E65100]", // Laranja
    location: "Befly Minascentro - Avenida Augusto de Lima, 785, Belo Horizonte - Minas Gerais",
    ...
    <div className="min-h-screen bg-[#dae4f8] pb-10 flex flex-col items-center">
      ...
      <main className="w-[95%] max-w-[1200px] mt-8 bg-white/50 backdrop-blur-sm border border-white/60 rounded-[30px] p-6 md:p-10 shadow-xl relative overflow-hidden">
        ...
        <div className="flex items-center gap-3 mb-8 relative z-10">
          ...
          <h1 className="text-3xl font-bold text-black font-serif">Eventos Favoritos</h1>
        </div>
        ...
        <div className="flex flex-col gap-6 relative z-10">
          {favorites.length > 0 ? (
            favorites.map((event) => (
              ...
              >
                ...
                <div className="flex-grow text-center md:text-left w-full">
                  <span className={`inline-block px-3 py-1 rounded-full text-xs font-bold mb-2 ${event.categoryColor}`}>
                    {event.categoryLabel}
                  </span>
                  <h3 className="text-xl font-extrabold text-gray-900 mb-2 leading-tight">
                    <Link to={`/event-details/${event.id}`} className="hover:text-[#0085D7] transition-colors">
                      {event.title}
                    </Link>
                  </h3>
                  ...
                  <div className="flex justify-between items-center mt-2">
                    <div className="bg-[#F3F0FA] px-3 py-1.5 rounded-lg flex items-center gap-2">
                      ...
                      <div className="flex flex-col leading-none">
                        <strong className="text-gray-800 text-sm">{event.tickets}</strong>
                        <span className="text-[9px] text-gray-500 font-bold uppercase">ingressos restantes</span>
                      </div>
                    </div>
            ...
            <div className="text-center py-20">
              <p className="text-gray-500 text-lg mb-4">Você ainda não tem eventos favoritos.</p>
              <Link to="/events" className="text-[#0085D7] font-bold hover:underline">
                Explorar eventos
              </Link>
            </div>
          )}
